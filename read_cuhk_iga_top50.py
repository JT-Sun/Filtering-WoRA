...
######################## 提取文件内的caption
# 定义JSON文件路径
gen_json = "/xxx/data/finetune/gene_attrs/g_i_g_a_attrs.json"
...
# print(caption_list[13])
# j = random.randint(0, len(caption_list) - 1)

# 指定图像文件夹路径
image_folder = "/xxx/images/CUHK-PEDES/gene_crop/i_g_a/"
...
for file_name in image_files:
    if file_name.endswith(".jpg"):  # 确保是图片文件
        image_name = os.path.splitext(file_name)[0]  # 去除文件扩展名
        image_id = int(image_name)  # 将文件名转换为数字
        image_name_list.append(image_id)

# print(image_name_list)
image_name_list = sorted(image_name_list,reverse=False)
...
for i in image_name_list:
    ## step 1
    result = []
    raw_image = Image.open("/xxx/images/CUHK-PEDES/gene_crop/i_g_a/"+ str(i) +".jpg").convert("RGB")
    caption= gen_caption_list[index]
    index += 1  # 增加索引以选择下一个 caption
...
output_file_path = "/xxx/output/iga.txt"
