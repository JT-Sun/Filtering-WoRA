...
from models.swin_transformer import SwinTransformer, interpolate_relative_pos_embed,LinearWithWoRAMerged,LinearWithWoRA
...
# def apply_dora_to_model_recursive(model, rank, alpha):
#     for name, child in model.named_children():
#         if isinstance(child, nn.Linear):
#             # 替换原始的nn.Linear层为包含DORA的版本
#             new_linear_with_dora = LinearWithDoRA(child, rank=rank, alpha=alpha)
#             setattr(model, name, new_linear_with_dora)
#         else:
#             # 递归处理子模块
#             apply_dora_to_model_recursive(child, rank, alpha)

def apply_Woramerge_to_model_recursive(model, rank, alpha, beta):
    for name, child in model.named_children():
        if isinstance(child, nn.Linear):
            # 替换原始的nn.Linear层为包含WORA的版本
            new_linear_with_dora = LinearWithWoRAMerged(child, rank=rank, alpha=alpha, beta=beta)
            setattr(model, name, new_linear_with_dora)
        else:
            # 递归处理子模块
            apply_Woramerge_to_model_recursive(child, rank, alpha, beta)
...
def build_vision_encoder(config, load_params=False):
    ...
    #########################################  apply WoRA for pretrain
    apply_Woramerge_to_model_recursive(vision_encoder, rank=8, alpha=8, beta=1)
    def freeze_linear_layers(model):
        for name, child in model.named_children():
            # 检查是否为LinearWithWoRA层
            if isinstance(child, LinearWithWoRAMerged):
                # 冻结LinearWithWoRA中原始线性层的参数
    ...
    freeze_linear_layers(vision_encoder)
...
class APTM(nn.Module):
    def __init__(self, config=None, load_vision_params=False, load_text_params=False,
                 ...
                 use_mlm_loss=False, use_bbox_loss=False, config_text=None):
        super().__init__()
        ...
        # self.dropout = nn.Dropout(p=0.9)  # 添加 dropout 层
        if ('pa100k_only_img_classifier' in config.keys()) and config['pa100k_only_img_classifier']:
            ...
            if use_bbox_loss:
                ...
                self.class_head = build_mlp(input_dim=self.text_width, output_dim=1)
    ...
    def get_matching_loss(self, image_embeds, image_atts, image_feat, text_embeds, text_atts, text_feat, idx=None):
        ...
        with torch.no_grad():
            ...
            correct_preds = torch.sum(preds == itm_labels)
            accuracy = correct_preds.float() / itm_labels.size(0)

            # # 打印准确率
            # accuracy_info = f"ITM Accuracy: {accuracy.item() * 100:.2f}%\n"
            # # print(f"ITM Accuracy: {accuracy.item() * 100:.2f}%")
            # # 将准确率信息写入文件
            # accuracy_file_path = "/public/home/wangmiao/sjt_new/APTM-A800/output/accuracy_itm_filetop50.txt"
            # with open(accuracy_file_path, "a") as file:
            #     file.write(accuracy_info)

        return itm_loss,accuracy

    def get_matching_loss_acc(self, image_embeds, image_atts, image_feat, text_embeds, text_atts, text_feat, idx=None):
        """
        Matching Loss with hard negatives
        """
        bs = image_embeds.size(0)
        ...
        cross_neg = self.get_cross_embeds(image_embeds_all, image_atts_all, text_embeds=text_embeds_all,
                                          text_atts=text_atts_all)[:, 0, :]
        # 应用 dropout
        # cross_pos = self.dropout(cross_pos)
        # cross_neg = self.dropout(cross_neg)
        label_smoothing = CrossEntropyLabelSmooth(epsilon=0.1, use_gpu=True)
        output = self.itm_head(torch.cat([cross_pos, cross_neg], dim=0))
        itm_labels = torch.cat([torch.ones(bs, dtype=torch.long),
                                torch.zeros(2 * bs, dtype=torch.long)], dim=0).to(image_embeds.device)

        # itm_loss = F.cross_entropy(output, itm_labels)
        itm_loss = label_smoothing(output, itm_labels)
        ######################## 计算准确率并保存到文件
        with torch.no_grad():
            preds = torch.argmax(output, dim=1)
            correct_preds = torch.sum(preds == itm_labels)
    ...
    def label_smooth_loss(self, inputs, targets):
        ...
        inputs_neg = []
        targets_neg = []
        # print("inputs size:", inputs.size())  # 打印inputs的尺寸
        # print("targets size:", targets.size())  # 打印targets的尺寸
        for b in range(bs):
            if targets[b] != -1:
    ...
    def get_contrastive_loss_attr(self, image_feat, text_feat, label):
        ...
        l = 0
        # print("logits size:", logits.size())  # 打印logits的尺寸
        # print("label size:", label.size())  # 打印label的尺寸
        for i in range(label.size(1)):
            left = 2 * i
            right = 2 * i + 2
            # print("i:", i)  # 打印当前的i值
            # print("label[:, i] size:", label[:, i].size())  # 打印label[:, i]的尺寸
    ...
    # def predict_logits(self, image_embeds, text_embeds, text_atts):
    #     assert image_embeds.size(0) == text_embeds.size(0)
    #
    #     # Get the cross-modal embeddings, which combine image and text features.
    #     # Assuming get_cross_embeds is a method that computes the cross-attention between image and text embeddings.
    #     cross_embeds = self.get_cross_embeds(image_embeds, torch.ones(image_embeds.shape[:2]).to(image_embeds.device),
    #                                          text_embeds=text_embeds, text_atts=text_atts)[:, 0, :]
    #     # 通过类别预测头计算logit
    #     output_logit = self.class_head(cross_embeds)
    #
    #     # 将logit转换为概率
    #     output_prob = output_logit.sigmoid()
    #
    #     return output_prob
    #
    # def get_logits(self, image_feat, text_feat):
    #     assert image_feat.size(-1) == self.embed_dim
    #     assert text_feat.size(-1) == self.embed_dim
    #     image_feat = F.normalize(image_feat, dim=-1)
    #     text_feat = F.normalize(text_feat, dim=-1)
    ...
    def get_bbox_loss(self, output_coord, target_bbox, logits, is_image=None):
        """
        ...
        # 过滤无效的边界框
        valid_boxes1 = boxes1[(boxes1 >= 0).all(dim=1) & ((boxes1[:, 2:] > boxes1[:, :2]).all(dim=1))]
        valid_boxes2 = boxes2[(boxes2 >= 0).all(dim=1) & ((boxes2[:, 2:] > boxes2[:, :2]).all(dim=1))]

        if valid_boxes1.size(0) > 0 and valid_boxes2.size(0) > 0:
            # 仅当两组边界框都有有效值时才计算 GIoU 损失
            loss_giou = 1 - torch.diag(box_ops.generalized_box_iou(valid_boxes1, valid_boxes2))  # bsz
        else:
            # 如果任一组边界框中没有有效值，则将 GIoU 损失设置为零
            loss_giou = torch.zeros(output_coord.size(0), device=output_coord.device)
        # boxes1 = box_ops.box_cxcywh_to_xyxy(output_coord)
        # boxes2 = target_bbox
        # loss_bbox = F.l1_loss(boxes1, boxes2, reduction='none')
        ...
        # 使用logits作为权重来计算损失
        weighted_loss_bbox = (loss_bbox * logits).sum() / logits.sum()
        weighted_loss_giou = (loss_giou * logits).sum() / logits.sum()

        return weighted_loss_bbox, weighted_loss_giou
        # # 使用logits作为权重来计算损失
        # weighted_loss_bbox = (loss_bbox * logits).sum()
        # weighted_loss_giou = (loss_giou * logits).sum()
        #
        # # 确保num_boxes不为零
        # if num_boxes > 0:
        #     # 使用num_boxes对加权损失进行规范化
        #     normalized_loss_bbox = weighted_loss_bbox / num_boxes
        #     normalized_loss_giou = weighted_loss_giou / num_boxes
        # else:
        #     normalized_loss_bbox = torch.tensor(0.0, device=output_coord.device)
        #     normalized_loss_giou = torch.tensor(0.0, device=output_coord.device)
        #
        # return normalized_loss_bbox, normalized_loss_giou

    # def get_bbox_loss_nologits(self, output_coord, target_bbox, logits, is_image=None):
    #     """
    #     Bounding Box Loss: L1 & GIoU
    #
    #     Args:
    ...
    #     # boxes1 = box_ops.box_cxcywh_to_xyxy(output_coord)
    #     # boxes2 = target_bbox
    #     # # print("boxes1 shape:", boxes1.shape)   # 50,4
    #     # # print("boxes2 shape:", boxes2.shape)   # 50,9,4
    #     # # print(boxes2)
    #     # # # boxes1_expanded = boxes1.unsqueeze(1).expand(-1, boxes2.size(1), -1)
    #     # # print(boxes1)
    #     # loss_bbox = F.l1_loss(boxes1, boxes2, reduction='none')
    #     if(boxes1 < 0).any():
    #         # print("1111111")
    ...
    #         # print(boxes2)
    #         loss_giou = torch.zeros(output_coord.size(0), device=output_coord.device)
    #     elif (boxes1[:, 2:] < boxes1[:, :2]).any() or (boxes2[:, 2:] < boxes2[:, :2]).any():
    #         # early check of degenerated boxes
    #         print("### (boxes1[:, 2:] < boxes1[:, :2]).any() or (boxes2[:, 2:] < boxes2[:, :2]).any()")
    #         loss_giou = torch.zeros(output_coord.size(0), device=output_coord.device)
    ...
    def get_bbox_loss_nologits(self, output_coord, target_bbox, logits, is_image=None):
        ...
        Args:
            output_coord: Predicted bounding box coordinates.
            target_bbox: Ground truth bounding box coordinates.
            logits: Logits for weighting loss (optional).
            is_image: Mask to differentiate between image and non-image data (optional).
        """
        loss_bbox = F.l1_loss(output_coord, target_bbox, reduction='none')  # bsz, 4
