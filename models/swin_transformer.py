...
class WoRALayer(nn.Module):   # WoRA layer
    def __init__(self, in_dim, out_dim, rank, alpha=8.0, beta=1.0):
        super().__init__()
        ...
        self.beta = nn.Parameter(torch.tensor(beta).float())
    def forward(self, x):
...
class LinearWithWoRA(nn.Module):
    def __init__(self, linear, rank, alpha):
        super().__init__()
        self.linear = linear
        self.lora = WoRALayer(linear.in_features, linear.out_features, rank, alpha)
...
class LinearWithWoRAMerged(nn.Module):
    def __init__(self, linear, rank, alpha=8.0, beta=1.0):
        super().__init__()
        self.linear = linear
        self.lora = WoRALayer(
            linear.in_features, linear.out_features, rank, alpha=alpha, beta=beta
        )
        # self.lambda_param = nn.Parameter(torch.zeros_like(self.linear.weight))
        self.m = nn.Parameter(
    ...
    def forward(self, x):
        ...
        lora = self.lora.A @ self.lora.B
        # beta* W + alpha * AB
        numerator = self.lora.beta * self.linear.weight + self.lora.alpha * lora.T
        denominator = numerator.norm(p=2, dim=0, keepdim=True)
        # 计算新权重的方向分量
        directional_component = numerator / denominator
        # 计算最终权重
    ...
    def __repr__(self):
        return (f'{self.__class__.__name__}('
                f'alpha={self.lora.alpha.item()}, '
                f'beta={self.lora.beta.item()})')
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, idx_to_group_img=None, image_atts=None):
