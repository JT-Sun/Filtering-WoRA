...
for m in range(0,44):
    # 定义JSON文件路径
    gen_json = "/xxx/data/finetune/gene_attrs/g_c_g_a_"+str(m)+"_attrs.json"
    ...
    for i in range(len(gen_caption_list)):
        ...
        raw_image = Image.open(
            "/xxx/images/CUHK-PEDES/gene_crop/c_g_a_" + str(m) + "/" + str(i) + ".jpg").convert(
            "RGB")
...
# print("gene_crop/c_g_a/"+ str(image_name) +".jpg")
    print(image_name)
    image_total.append(image_name)
print(image_total)

output_file_path = "/xxx/output/c_g_a.txt"

with open(output_file_path, "w") as output_file:
    for item in image_total:
