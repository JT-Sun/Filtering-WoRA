...
def mlm_new(text, text_input, tokenizer, mask_generator, config, pa100k=False):
    if pa100k:
        text_masked = tokenizer(text, padding='longest', max_length=config['max_tokens'], return_tensors="pt")
    else:
        text_masked = tokenizer(text, padding='max_length', truncation=True, max_length=config['max_tokens'],
                                    return_tensors="pt")
    ...
    masked_pos = torch.empty((text_ids_masked.shape[0], config['max_masks']), dtype=torch.int64)
    masked_ids = torch.empty((text_ids_masked.shape[0], config['max_masks']), dtype=torch.long)
    ...
    for index, text_id in enumerate(text_ids_masked):
        ...
        n_pad = config['max_masks'] - len(masked_ids_)
        masked_pos_ = masked_pos_ + [0] * n_pad
        masked_pos_ = torch.tensor(masked_pos_, dtype=torch.int64)
        masked_ids_ = masked_ids_ + [-100] * n_pad
        masked_ids_ = torch.tensor(masked_ids_, dtype=torch.long)
        masked_pos[index] = masked_pos_
        masked_ids[index] = masked_ids_
