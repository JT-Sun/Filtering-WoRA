...
from dataset.re_dataset import re_train_dataset, re_train_dataset_iter, re_test_dataset, re_test_dataset_icfg, re_train_dataset_attr, \
    re_test_dataset_attr, re_train_dataset_addbox, re_train_dataset_addbox_iter
...
def create_dataset(dataset, config, evaluate=False):
    ...
    #     if evaluate:
    #         return None, val_dataset, test_dataset
    #     train_dataset = re_train_dataset_iter(config, train_transform, pre_transform, config['train_file'], rank=int(0),
    #                                        world_size=int(4), shuffle=True, repeat=True)
    #     train_dataset_box = re_train_dataset_addbox_iter(config, train_transform, pre_transform, config['train_file'], rank=int(0),
    #                                         world_size=int(4), shuffle=True, repeat=True)
    #     return train_dataset, train_dataset_box, val_dataset, test_dataset
    else:
        if config['box']:
            test_dataset = re_test_dataset(config['test_file'], config, test_transform)
            val_dataset = re_test_dataset(config['val_file'], config, test_transform)
            if evaluate:
                return None, val_dataset, test_dataset
            train_dataset_addbox = re_train_dataset_addbox(config, train_transform, pre_transform)
            return train_dataset_addbox, val_dataset, test_dataset
            # test_dataset = re_test_dataset(config['test_file'], config, test_transform)
            # val_dataset = re_test_dataset(config['val_file'], config, test_transform)
            # if evaluate:
            #     return None, None, val_dataset, test_dataset
            # train_dataset = re_train_dataset_iter(config, train_transform, pre_transform, config['train_file'], rank=int(0),
            #                                    world_size=int(4), shuffle=True, repeat=True)
            # train_dataset_box = re_train_dataset_addbox_iter(config, train_transform, pre_transform, config['train_file'], rank=int(0),
            #                                     world_size=int(4), shuffle=True, repeat=True)
