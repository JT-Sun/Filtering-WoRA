...
def train(model, data_loader, optimizer, tokenizer, epoch, device, scheduler, config,
          mask_generator=None, accelerator=None):
    ...
    if config['eda']:
        for i, (image, text, text_eda, idx) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
            ...
            if config['mlm']:
                ...
                loss_itc, loss_itm, loss_mlm, accuracy_info = model(image, text_input.input_ids, text_input.attention_mask,
                ...
                loss_itc, loss_itm, accuracy_info = model(image, text_input.input_ids, text_input.attention_mask, idx=idx,
                                           text_ids_eda=text_input_eda.input_ids,
                                           text_atts_eda=text_input_eda.attention_mask)
            ...
            metric_logger.update(accuracy=accuracy_info.item()*100)
        ...
        if wandb.run:
            wandb.log({
                "loss_itc": metric_logger.meters['loss_itc'].global_avg,
                "loss_itm": metric_logger.meters['loss_itm'].global_avg,
                "loss_mlm": metric_logger.meters['loss_mlm'].global_avg if config['mlm'] else 0,
                "learning_rate": metric_logger.meters['lr'].global_avg,
                "epoch": epoch,
                "accuracy" : metric_logger.meters['accuracy'].global_avg
            })
    else:
...
def train_attr(model, data_loader, optimizer, tokenizer, epoch, device, scheduler, config,
               mask_generator=None, accelerator=None):
    model.train()
    ...
    for i, (image, text, idx, label) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if config['mlm']:
            ...
            loss_itc, loss_itm, loss_mlm, loss_attr, accuracy_info = model(image, text_input.input_ids, text_input.attention_mask,
            ...
            loss_itc, loss_itm, loss_attr, accuracy_info = model(image, text_input.input_ids, text_input.attention_mask, idx=idx,
                                                  attr_text_ids=attr_input.input_ids,
                                                  attr_text_atts=attr_input.attention_mask,
                                                  label=label)
    ...
    if wandb.run:
        wandb.log({
            ...
            "learning_rate": metric_logger.meters['lr'].global_avg,
            "epoch": epoch,
            "loss_attr": metric_logger.meters['loss_attr'].global_avg,
            "accuracy": metric_logger.meters['accuracy'].global_avg
        })
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
